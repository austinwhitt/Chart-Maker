...
class MarketMetricsGenerator:
    ...
    def preprocess_data(self, data):
        """Cleans the data by removing dollar signs, commas, and converting to float."""
        clean_data = data.copy()
        try:
            clean_data.iloc[:, 1:] = clean_data.iloc[:, 1:].replace(
                r'[\$,]', '', regex=True).replace('', None).astype(float)
        except Exception as e:
            print(f"Error during preprocessing: {e}")
        return clean_data

    def calculate_changes(self, data, recent_month, previous_month, last_year_month):
        """Calculates MoM and YoY changes while avoiding division by zero."""
        ...
        changes['MoM Change (%)'] = (
            (data[recent_month] - data[previous_month]) / data[previous_month].replace(0, float('nan'))
        ) * 100
        changes['YoY Change (%)'] = (
            (data[recent_month] - data[last_year_month]) / data[last_year_month].replace(0, float('nan'))
        ) * 100

        # Replace infinite or NaN values with 0
        changes['MoM Change (%)'].replace([float('inf'), -float('inf'), float('nan')], 0, inplace=True)
        changes['YoY Change (%)'].replace([float('inf'), -float('inf'), float('nan')], 0, inplace=True)
    ...
    def generate_json_for_metric(self, metric):
        try:
            ...
            if not os.path.exists(city_path) or not os.path.exists(county_path):
